...
TextRenderer::TextRenderer(std::string text, std::string font, int
	...
	for (GLubyte i = 0; i < 128; i++) {
		...
		if (FT_Load_Char(face, i, FT_LOAD_RENDER)) {
			std::cout << "ERROR::FREETYTPE: Failed to loadGlyph"
				<<
				std::endl;
			continue;
		}
...
void TextRenderer::draw() {
	...
	for (c = text.begin(); c != text.end(); c++) {
		...
		// Now advance cursors for next glyph (note that advanceis number of 1 / 64 pixels)
		// Bitshift by 6 to get value in pixels (2^6 = 64 (divideamount of 1 / 64th pixels by 64 to get amount of pixels))
